...
## NEEDS
### THE PRIZE
| id | label | type | locked | ask | not-sure line | diggable | when | after | unit |
|---|---|---|---|---|---|---|---|---|---|
| prize_type | Prize type | dropdown: movie / gig / sport / other | yes | Is it movie passes, gig tickets, sports tickets, or something else? | | no | | |
| prize_name | Event | text | yes | What's the event? | | no | label and ask vary by type — see below | |
| winners | Prize | number | yes | How many? | | no | | | @type |
| venue | Venue | text | yes | Where is it? | I can hunt for the venue myself. | yes | prize_type in gig, sport | |
| event_date | When? | date | yes | When is it? | | yes | prize_type in gig, sport | closes | |
**The type row shows only while it's unknown** — once the robot (or the human) has it, it leaves the card; it keeps working backstage (which clauses propose, the prize line, which facts appear). **Prize** reads as word + noun ("Ten double passes"); the noun is the type's `counts:` line below — SET UP owns the word. `unit: @type` in the table is what asks for it.
...
| drawn | Prize drawn | date | | yes | When? | | no | closes | next working day after closes; a human date wins but can't land before closes |
